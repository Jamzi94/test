Absolument. Voici le fichier `deploy.sh` complet, intégrant les corrections pour le rendre plus robuste et dynamique.

Les deux modifications principales sont :

1.  **Vérification des privilèges administrateur** : Une nouvelle fonction `check_admin_rights` est ajoutée et appelée au début pour s'assurer que le script a les droits nécessaires pour installer des logiciels comme Chocolatey.
2.  **Correction du chemin d'accès `choco`** : La ligne qui installe `aria2c` a été modifiée pour inclure explicitement le chemin de Chocolatey, résolvant ainsi le problème où la commande n'est pas trouvée immédiatement après l'installation.

-----

### Fichier `deploy.sh` corrigé et complet

```bash
#!/bin/bash
# ==============================================================================
#      J.A.M.Z.I. AI STACK - DEPLOYMENT & MANAGEMENT FRAMEWORK v50.0
# ==============================================================================
...
function log_ok()     { echo -e "  $ICON_OK ${1}"; }
function log_warn()   { echo -e "  $ICON_WARN ${C_YELLOW}AVERTISSEMENT:${C_RESET} ${1}"; }
function log_error()  { echo -e "\n$ICON_ERROR ${C_RED}ERREUR:${C_RESET} ${1}\n" >&2; exit 1; }

# --- GESTION DES INTERRUPTIONS (Préservé de v44.0) ---
function cleanup {
  echo -e "\n  $ICON_WARN ${C_YELLOW}Opération annulée. Nettoyage...${C_RESET}"
  if [ -f "$BASE_DIR/docker-compose.yml" ]; then
    cd "$BASE_DIR" && docker compose down --remove-orphans >/dev/null 2>&1
...
# CORRECTION: Nouvelle fonction pour vérifier les droits admin
# Vérifie que le script est bien exécuté avec les privilèges administrateur
function check_admin_rights() {
    echo -e "  ${C_DIM}Vérification des privilèges administrateur...${C_RESET}"
    local is_admin
    # Exécute une commande PowerShell pour vérifier si le script est lancé en tant qu'administrateur.
    is_admin=$(powershell.exe -NoProfile -Command "([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)")

    # Nettoie la sortie de PowerShell pour ne garder que "True" ou "False"
    is_admin=$(echo "$is_admin" | tr -d '[:space:]')

    if [[ "$is_admin" != "True" ]]; then
        log_error "Ce script nécessite des privilèges administrateur pour installer des dépendances comme Chocolatey.
    ${C_YELLOW}Veuillez fermer ce terminal, puis le relancer en faisant un clic droit sur son icône et en choisissant 'Exécuter en tant qu'administrateur'.${C_RESET}"
    fi
    log_ok "Privilèges administrateur détectés."
}

# Vérifie l'environnement système et charge les configurations
function check_environment() {
    log_header "PHASE 1 : VÉRIFICATION DE L'ENVIRONNEMENT"
    check_admin_rights # CORRECTION: Appel de la nouvelle fonction
    ...
    if ! command -v aria2c &>/dev/null; then
        log_warn "Commande 'aria2c' introuvable. Tentative d'installation automatique via Chocolatey..."
        
        # Vérifie si choco est disponible via PowerShell
        if ! powershell.exe -Command "Get-Command choco -ErrorAction SilentlyContinue" &>/dev/null; then
            log_warn "Chocolatey non trouvé. Tentative d'installation de Chocolatey..."
            echo -e "  ${C_YELLOW}L'installation de Chocolatey peut nécessiter des privilèges administrateur et une confirmation.${C_RESET}
"
            echo -e "  ${C_YELLOW}Veuillez suivre les instructions si une fenêtre PowerShell s'ouvre.${C_RESET}"
            
            # Exécute l'installation de Chocolatey via PowerShell
            ...
            if [ $? -ne 0 ]; then
                log_error "L'installation de Chocolatey a échoué. Veuillez l'installer manuellement et réessayer."
            fi
            log_ok "Chocolatey installé (ou déjà présent)."
        else
            log_ok "Chocolatey déjà présent."
        fi

        echo -e "  ${C_DIM}Installation de aria2c via Chocolatey...${C_RESET}"
        # CORRECTION: Ajout explicite du chemin de Choco au PATH de la session pour éviter les erreurs post-installation
        powershell.exe -Command "$env:Path += ';C:\ProgramData\chocolatey\bin'; choco install aria2 -y"
        if [ $? -ne 0 ]; then
            log_error "L'installation de 'aria2c' via Chocolatey a échoué. Veuillez l'installer manuellement et réessayer."
        fi
        log_ok "Commande 'aria2c' installée."
    else
...
# --- FONCTIONS D'INSTALLATION WSL2 + NVIDIA CUDA TOOLKIT ---
function install_wsl_cuda() {
    log_header "INSTALLATION WSL2 + NVIDIA CUDA TOOLKIT"
    echo -e "${C_BOLD}Ce processus va vous guider pour installer et configurer WSL2 et l'accès GPU NVIDIA sur votre système Windows.${C_RESET}"
    echo -e "${C_YELLOW}ATTENTION : Ce processus nécessite des redémarrages de votre machine. Suivez attentivement les instructions.${C_RESET}"
    read -p "Appuyez sur Entrée pour commencer, ou Ctrl+C pour annuler." 

    log_header "ÉTAPE 1 : ACTIVER LES FONCTIONNALITÉS WINDOWS REQUISES"
    echo -e "  ${C_DIM}Activation des fonctionnalités 'Sous-système Windows pour Linux' et 'Plateforme de machine virtuelle'...${C_RESET}"
    ...
    log_ok "Fonctionnalités Windows activées. Un redémarrage peut être nécessaire."
    echo -e "${C_YELLOW}Si Windows vous demande de redémarrer, faites-le MAINTENANT, puis relancez ce script après le redémarrage.${C_RESET}"
    read -p "Appuyez sur Entrée pour continuer (après un éventuel redémarrage)." 

    log_header "ÉTAPE 2 : MISE À JOUR ET DÉFINITION DE WSL2"
    echo -e "  ${C_DIM}Mise à jour du noyau WSL et définition de WSL2 comme version par défaut...${C_RESET}"
    wsl.exe --update
    wsl.exe --set-default-version 2
    log_ok "WSL2 mis à jour et configuré comme version par défaut."
    read -p "Appuyez sur Entrée pour continuer." 

    log_header "ÉTAPE 3 : INSTALLATION DU PILOTE NVIDIA COMPATIBLE WSL2"
    echo -e "${C_YELLOW}IMPORTANT : Pour que Docker Desktop puisse utiliser votre GPU, vous devez avoir le dernier pilote NVIDIA compatible WSL2 installé sur votre Windows.${C_RESET}"
    echo -e "${C_CYAN}    Téléchargez le pilote le plus récent depuis le site officiel de NVIDIA :${C_RESET}"
    echo -e "${C_WHITE}    https://www.nvidia.com/drivers/ (recherchez les pilotes pour votre carte graphique, assurez-vous qu'ils supportent WSL2).${C_RESET}"
    echo -e "${C_YELLOW}    Installez le pilote et redémarrez votre machine si nécessaire. Relancez ce script après le redémarrage.${C_RESET}"
    read -p "Appuyez sur Entrée après avoir vérifié/mis à jour votre pilote NVIDIA." 

    log_header "ÉTAPE 4 : CONFIGURATION DE DOCKER DESKTOP"
    echo -e "${C_YELLOW}IMPORTANT : Configurez Docker Desktop pour utiliser le backend WSL2 et activer l'intégration GPU.${C_RESET}"
    echo -e "${C_CYAN}    1. Ouvrez Docker Desktop.\n    2. Allez dans Settings (⚙️) -> General, et assurez-vous que 'Use the WSL 2 based engine' est coché.\n    3. Allez dans Settings -> Resources -> GPU, et assurez-vous que 'Enable GPU virtualization' est coché.${C_RESET}"
    read -p "Appuyez sur Entrée après avoir configuré Docker Desktop." 

    log_header "ÉTAPE 5 : VÉRIFICATION FINALE DE L'ACCÈS GPU DANS DOCKER"
    echo -e "  ${C_DIM}Exécution d'un test Docker pour vérifier l'accès au GPU...${C_RESET}"
    if docker run --rm --gpus all nvidia/cuda:12.4.1-base nvidia-smi &>/dev/null; then
        log_ok "Accès au GPU NVIDIA via Docker fonctionnel !"
    else
        log_error "L'accès au GPU NVIDIA via Docker a échoué. Veuillez revoir les étapes précédentes et les paramètres de Docker Desktop."
    fi
    read -p "Appuyez sur Entrée pour continuer." 

    log_ok "Installation de WSL2 + NVIDIA CUDA Toolkit terminée avec succès !"
    echo -e "${C_GREEN}${C_BOLD}Votre environnement Windows est maintenant prêt pour l'IA avec accélération GPU.${C_RESET}"
}
